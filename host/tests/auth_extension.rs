...
//!
//! The extension lives in `sqlink-extension/runtimes/wasmtime/
...
//! `sqlink-extension` repo, matching the convention in
//! `host/tests/load.rs`.
...
fn auth_ext_path() -> Option<PathBuf> {
    ...
    let candidates = [
        "../../sqlink-extension/target/wasm32-wasip1/release/auth_extension.wasm",
        "../sqlink-extension/target/wasm32-wasip1/release/auth_extension.wasm",
    ];
    for c in candidates {
...
async fn dispatches_authorize_through_loaded_extension() {
    let Some(path) = auth_ext_path() else {
        eprintln!("skipping: auth_extension.wasm not found (build sqlink-extension's auth-extension)");
        return;
    };
