    ...
    impl MetadataGuest for Ext {
        fn describe() -> Manifest {
            ...
            Manifest {
                ...
                preferred_prefix: Some("iban".into()),
                prefix_expansion: Some("com.tegmentum.sqlink.ext.iban".into()),
            }
        }
    }
    ...
    impl ScalarFunctionGuest for Ext {
        fn call(func_id: u64, args: Vec<SqlValue>) -> Result<SqlValue, String> {
            ...
            match func_id {
                ...
                FID_COUNTRY => Ok(country(&raw).map(SqlValue::Text).unwrap_or(SqlValue::Null)),
                FID_CHECK_DIGITS => Ok(check_digits(&raw)
                    .map(SqlValue::Text)
                    .unwrap_or(SqlValue::Null)),
                FID_BBAN => Ok(bban(&raw).map(SqlValue::Text).unwrap_or(SqlValue::Null)),
