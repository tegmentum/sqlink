//! `aba` — US ABA / bank routing-number checks.
//!
//! THIN, GENERATED sqlink (`sqlite:extension`) shim: a
//! `wit_bindgen::generate!` block plus one
...
//! in datalink `aba-core`; the registration ABI, func-id dispatch, and
//! the `SqlValue` / `SqlValueOwned` marshalling are derived from the
//! core's `declare!` table. Replaces the previously hand-maintained
//! `lib.rs` + `embed.rs` copies (write-once dedup; the same algorithm is
//! no longer kept in three places).
