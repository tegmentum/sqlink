    ...
    match func_id {
        FID_REGEXP | FID_REGEXP_LIKE => {
            ...
            let re = Regex::new(pattern)
                .map_err(|e| format!("regexp: bad pattern: {e}"))?;
        ...
        FID_REGEXP_SUBSTR => {
            ...
            let re = Regex::new(pattern)
                .map_err(|e| format!("regexp_substr: bad pattern: {e}"))?;
        ...
        FID_REGEXP_REPLACE => {
            ...
            let re = Regex::new(pattern)
                .map_err(|e| format!("regexp_replace: bad pattern: {e}"))?;
...
const SCALARS: &[ScalarSpec] = &[
    ScalarSpec { func_id: FID_REGEXP,         name: b"regexp\0",         num_args: 2, deterministic: true },
    ScalarSpec { func_id: FID_REGEXP_LIKE,    name: b"regexp_like\0",    num_args: 2, deterministic: true },
    ScalarSpec { func_id: FID_REGEXP_SUBSTR,  name: b"regexp_substr\0",  num_args: 2, deterministic: true },
    ScalarSpec { func_id: FID_REGEXP_REPLACE, name: b"regexp_replace\0", num_args: 3, deterministic: true },
];
