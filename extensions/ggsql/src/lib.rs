//! `ggsql` / VISUALIZE parser extension — sqlink (`sqlite:extension`) port.
//!
//! THIN, GENERATED shim: a `wit_bindgen::generate!` block plus one
//! `datalink_extcore::sqlite_shim!`. All logic + the capability surface
//! live ONCE in datalink `ggsql-core`; the manifest, func-id dispatch,
...
//! `ggsql-core` declares. This component therefore loads as a plain
...
mod wasm_export {
    ...
    datalink_extcore::sqlite_shim! {
        ...
        prefix_expansion = "com.tegmentum.sqlink.ext.ggsql";
    }
}
