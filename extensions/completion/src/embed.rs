...
unsafe fn comp_make_cursor(_vtab_state: *mut (), db: *mut libsqlite3_sys::sqlite3) -> *mut () {
...
unsafe fn gather_all(db: *mut libsqlite3_sys::sqlite3) -> Vec<(String, i64)> {
    let mut out: Vec<(String, i64)> =
        Vec::with_capacity(KEYWORDS.len() + PRAGMAS.len() + FUNCTIONS.len() + COLLATIONS.len());
    for k in KEYWORDS {
...
fn filter_candidates(all: Vec<(String, i64)>, prefix: &str) -> Vec<(String, i64)> {
