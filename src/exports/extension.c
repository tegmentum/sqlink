 ...
 */
static void sqlite_value_to_wit(sqlite3_value *val, sqlite_wasm_extension_callbacks_sql_value_t *out) {
    ...
    switch (type) {
        case SQLITE_INTEGER:
            out->value_type = SQLITE_WASM_EXTENSION_VALUE_TYPE_INTEGER;
            out->int_value.is_some = true;
            out->int_value.val = sqlite3_value_int64(val);
            break;
        case SQLITE_FLOAT:
            out->value_type = SQLITE_WASM_EXTENSION_VALUE_TYPE_FLOAT;
            out->float_value.is_some = true;
            out->float_value.val = sqlite3_value_double(val);
            break;
        case SQLITE_TEXT:
            out->value_type = SQLITE_WASM_EXTENSION_VALUE_TYPE_TEXT;
            ...
            break;
        case SQLITE_BLOB:
            out->value_type = SQLITE_WASM_EXTENSION_VALUE_TYPE_BLOB;
        ...
        case SQLITE_NULL:
        default:
            out->value_type = SQLITE_WASM_EXTENSION_VALUE_TYPE_NULL;
            break;
    }
}
 ...
 */
static void wit_value_to_result(sqlite3_context *ctx, sqlite_wasm_extension_callbacks_sql_value_t *val) {
    switch (val->value_type) {
        case SQLITE_WASM_EXTENSION_VALUE_TYPE_INTEGER:
            ...
            break;
        case SQLITE_WASM_EXTENSION_VALUE_TYPE_FLOAT:
            ...
            break;
        case SQLITE_WASM_EXTENSION_VALUE_TYPE_TEXT:
            ...
            break;
        case SQLITE_WASM_EXTENSION_VALUE_TYPE_BLOB:
            ...
            break;
        case SQLITE_WASM_EXTENSION_VALUE_TYPE_NULL:
        default:
            sqlite3_result_null(ctx);
            break;
    }
...
static void scalar_function_callback(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    /* Build args list */
    sqlite_wasm_extension_callbacks_list_sql_value_t args;
    args.len = argc;
    args.ptr = (sqlite_wasm_extension_callbacks_sql_value_t *)
        malloc(argc * sizeof(sqlite_wasm_extension_callbacks_sql_value_t));
    ...
    sqlite_wasm_extension_callbacks_sql_value_t result_val;
    sqlite_extensible_string_t result_err;
    bool success = sqlite_wasm_extension_callbacks_on_scalar_function(function_id, &args, &result_val, &result_err);
...
static void aggregate_step_callback(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    /* Call the imported callback */
    sqlite_wasm_extension_callbacks_on_aggregate_step(function_id, *agg_ctx, &args);
...
static void aggregate_finalize_callback(sqlite3_context *ctx) {
    ...
    sqlite_wasm_extension_callbacks_sql_value_t result_val;
    sqlite_extensible_string_t result_err;
    bool success = sqlite_wasm_extension_callbacks_on_aggregate_finalize(function_id, context_id, &result_val, &result_err);
...
static int collation_compare_callback(void *user_data, int len1, const void *str1,
    ...
    return sqlite_wasm_extension_callbacks_on_collation_compare(collation_id, &a, &b);
}
...
static void update_hook_callback(void *user_data, int op, const char *database,
    ...
    sqlite_wasm_extension_callbacks_update_type_t update_type;
    switch (op) {
        case SQLITE_INSERT:
            update_type = SQLITE_WASM_EXTENSION_UPDATE_TYPE_INSERT;
            break;
        case SQLITE_UPDATE:
            update_type = SQLITE_WASM_EXTENSION_UPDATE_TYPE_UPDATE;
            break;
        case SQLITE_DELETE:
            update_type = SQLITE_WASM_EXTENSION_UPDATE_TYPE_DELETE;
            break;
        default:
            return;
    }
    ...
    sqlite_wasm_extension_callbacks_on_update(hook_id, update_type, &db_str, &table_str, rowid);
}
...
static int commit_hook_callback(void *user_data) {
    uint64_t hook_id = (uint64_t)(uintptr_t)user_data;
    return sqlite_wasm_extension_callbacks_on_commit(hook_id) ? 1 : 0;
}
...
static void rollback_hook_callback(void *user_data) {
    uint64_t hook_id = (uint64_t)(uintptr_t)user_data;
    sqlite_wasm_extension_callbacks_on_rollback(hook_id);
}
...
static int authorizer_callback(void *user_data, int action, const char *arg1,
    ...
    sqlite_wasm_extension_callbacks_auth_action_t wit_action;
    switch (action) {
        case SQLITE_CREATE_INDEX: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_CREATE_INDEX; break;
        case SQLITE_CREATE_TABLE: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_CREATE_TABLE; break;
        case SQLITE_CREATE_TEMP_INDEX: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_CREATE_TEMP_INDEX; break;
        case SQLITE_CREATE_TEMP_TABLE: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_CREATE_TEMP_TABLE; break;
        case SQLITE_CREATE_TEMP_TRIGGER: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_CREATE_TEMP_TRIGGER; break;
        case SQLITE_CREATE_TEMP_VIEW: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_CREATE_TEMP_VIEW; break;
        case SQLITE_CREATE_TRIGGER: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_CREATE_TRIGGER; break;
        case SQLITE_CREATE_VIEW: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_CREATE_VIEW; break;
        case SQLITE_DELETE: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_DELETE; break;
        case SQLITE_DROP_INDEX: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_DROP_INDEX; break;
        case SQLITE_DROP_TABLE: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_DROP_TABLE; break;
        case SQLITE_DROP_TEMP_INDEX: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_DROP_TEMP_INDEX; break;
        case SQLITE_DROP_TEMP_TABLE: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_DROP_TEMP_TABLE; break;
        case SQLITE_DROP_TEMP_TRIGGER: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_DROP_TEMP_TRIGGER; break;
        case SQLITE_DROP_TEMP_VIEW: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_DROP_TEMP_VIEW; break;
        case SQLITE_DROP_TRIGGER: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_DROP_TRIGGER; break;
        case SQLITE_DROP_VIEW: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_DROP_VIEW; break;
        case SQLITE_INSERT: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_INSERT; break;
        case SQLITE_PRAGMA: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_PRAGMA; break;
        case SQLITE_READ: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_READ; break;
        case SQLITE_SELECT: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_SELECT; break;
        case SQLITE_TRANSACTION: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_TRANSACTION; break;
        case SQLITE_UPDATE: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_UPDATE; break;
        case SQLITE_ATTACH: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_ATTACH; break;
        case SQLITE_DETACH: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_DETACH; break;
        case SQLITE_ALTER_TABLE: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_ALTER_TABLE; break;
        case SQLITE_REINDEX: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_REINDEX; break;
        case SQLITE_ANALYZE: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_ANALYZE; break;
        case SQLITE_CREATE_VTABLE: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_CREATE_VTABLE; break;
        case SQLITE_DROP_VTABLE: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_DROP_VTABLE; break;
        case SQLITE_FUNCTION: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_FUNCTION; break;
        case SQLITE_SAVEPOINT: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_SAVEPOINT; break;
        case SQLITE_RECURSIVE: wit_action = SQLITE_WASM_EXTENSION_AUTH_ACTION_RECURSIVE; break;
    ...
    sqlite_wasm_extension_callbacks_auth_result_t result =
        sqlite_wasm_extension_callbacks_on_authorize(auth_id, wit_action,
            arg1 ? &str_arg1 : NULL,
            arg2 ? &str_arg2 : NULL,
    ...
    switch (result) {
        case SQLITE_WASM_EXTENSION_AUTH_RESULT_OK: return SQLITE_OK;
        case SQLITE_WASM_EXTENSION_AUTH_RESULT_DENY: return SQLITE_DENY;
        case SQLITE_WASM_EXTENSION_AUTH_RESULT_IGNORE: return SQLITE_IGNORE;
        default: return SQLITE_OK;
    }
}
 ...
 */
static void set_extension_error(exports_sqlite_wasm_extension_extension_error_t *err,
 ...
 */
static int wit_to_sqlite_function_flags(exports_sqlite_wasm_extension_function_flags_t flags) {
    int sqlite_flags = SQLITE_UTF8;
    if (flags & EXPORTS_SQLITE_WASM_EXTENSION_FUNCTION_FLAGS_DETERMINISTIC) {
        sqlite_flags |= SQLITE_DETERMINISTIC;
    }
    if (flags & EXPORTS_SQLITE_WASM_EXTENSION_FUNCTION_FLAGS_DIRECT_ONLY) {
...
bool exports_sqlite_wasm_extension_register_scalar_function(
    exports_sqlite_wasm_extension_db_handle_t db,
    sqlite_extensible_string_t *name,
    int32_t num_args,
    exports_sqlite_wasm_extension_function_flags_t flags,
    uint64_t function_id,
    exports_sqlite_wasm_extension_function_handle_t *ret,
...
bool exports_sqlite_wasm_extension_unregister_function(
    exports_sqlite_wasm_extension_function_handle_t handle,
    exports_sqlite_wasm_extension_extension_error_t *err
) {
...
bool exports_sqlite_wasm_extension_register_aggregate_function(
    exports_sqlite_wasm_extension_db_handle_t db,
    sqlite_extensible_string_t *name,
    int32_t num_args,
    exports_sqlite_wasm_extension_function_flags_t flags,
...
bool exports_sqlite_wasm_extension_register_collation(
    exports_sqlite_wasm_extension_db_handle_t db,
    sqlite_extensible_string_t *name,
    uint64_t collation_id,
    exports_sqlite_wasm_extension_collation_handle_t *ret,
    exports_sqlite_wasm_extension_extension_error_t *err
) {
    sqlite3 *pdb = HANDLE_TO_DB(db);
...
bool exports_sqlite_wasm_extension_unregister_collation(
    exports_sqlite_wasm_extension_collation_handle_t handle,
    exports_sqlite_wasm_extension_extension_error_t *err
) {
...
bool exports_sqlite_wasm_extension_set_update_hook(
    exports_sqlite_wasm_extension_db_handle_t db,
    uint64_t hook_id,
    exports_sqlite_wasm_extension_hook_handle_t *ret,
    exports_sqlite_wasm_extension_extension_error_t *err
) {
    sqlite3 *pdb = HANDLE_TO_DB(db);
...
bool exports_sqlite_wasm_extension_remove_update_hook(
    exports_sqlite_wasm_extension_hook_handle_t handle,
    exports_sqlite_wasm_extension_extension_error_t *err
) {
...
bool exports_sqlite_wasm_extension_set_commit_hook(
    exports_sqlite_wasm_extension_db_handle_t db,
    uint64_t hook_id,
    exports_sqlite_wasm_extension_hook_handle_t *ret,
    exports_sqlite_wasm_extension_extension_error_t *err
...
bool exports_sqlite_wasm_extension_remove_commit_hook(
    exports_sqlite_wasm_extension_hook_handle_t handle,
    exports_sqlite_wasm_extension_extension_error_t *err
) {
    if (handle >= (uint64_t)g_hook_count) {
...
bool exports_sqlite_wasm_extension_set_rollback_hook(
    exports_sqlite_wasm_extension_db_handle_t db,
    uint64_t hook_id,
    exports_sqlite_wasm_extension_hook_handle_t *ret,
    exports_sqlite_wasm_extension_extension_error_t *err
...
bool exports_sqlite_wasm_extension_remove_rollback_hook(
    exports_sqlite_wasm_extension_hook_handle_t handle,
    exports_sqlite_wasm_extension_extension_error_t *err
) {
    if (handle >= (uint64_t)g_hook_count) {
...
bool exports_sqlite_wasm_extension_set_busy_timeout(
    exports_sqlite_wasm_extension_db_handle_t db,
    int32_t ms,
    exports_sqlite_wasm_extension_extension_error_t *err
) {
...
bool exports_sqlite_wasm_extension_set_authorizer(
    exports_sqlite_wasm_extension_db_handle_t db,
    uint64_t auth_id,
    exports_sqlite_wasm_extension_hook_handle_t *ret,
    exports_sqlite_wasm_extension_extension_error_t *err
...
bool exports_sqlite_wasm_extension_remove_authorizer(
    exports_sqlite_wasm_extension_hook_handle_t handle,
    exports_sqlite_wasm_extension_extension_error_t *err
) {
    if (handle >= (uint64_t)g_hook_count) {
