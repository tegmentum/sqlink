...
export function buildCliHostHandlers(opts) {
  ...
  // ────────────── bundles (bridged via cas conn) ──────────────
  //
  // v1.5 round 3: the composed cli's bundle-cli extension imports
  // `sqlite:extension/bundles` for `.bundle save/list/show/delete/...`
  // dot-cmds. Native sqlink-host services this surface directly
  // through `sqlite_cas_cache::bundles_exec` free functions against
  // a rusqlite connection rooted at `~/.cache/sqlink/cas.db`. In the
  // browser composed runtime there is no host process — we drive the
  // same CRUD inline against the composed binary's NEW cas connection
  // via `dispatch-bridge.bridged-execute-cas`.
  //
  // SQL strings here are a transcription of
  // `sqlite-cas-cache/src/bundles_exec.rs`'s `pub const` SQL
  ...
  // Helper: build a jco-shaped sql-value variant.
  function sqlVal(tag, val) {
    return { tag, val }
  }
  function sqlInt(n) {
    return { tag: 'integer', val: BigInt(n) }
  }
  function sqlText(s) {
    return { tag: 'text', val: String(s) }
  }
  function sqlNull() {
    return { tag: 'null' }
  }

  // Helper: extract a row column as a JS-native value (integer ->
  // BigInt; text -> string; null -> undefined; rest passthrough).
  ...
  function colText(row, idx) {
    const v = row[idx]
    if (!v) return ''
    if (v.tag === 'text') return String(v.val ?? '')
    return ''
  }
  function colOptText(row, idx) {
    const v = row[idx]
    if (!v || v.tag === 'null') return undefined
    if (v.tag === 'text') return String(v.val ?? '')
    return undefined
  }
  function nowSecs() {
  ...
  // Hydrate a __cas_bundle row into a `bundle-summary` record (with
  // member-count + binary-count counted via separate queries — these
  // mirror COUNT_MEMBERS_SQL / COUNT_BINARIES_SQL from bundles_exec).
  function summaryFromRow(row) {
    const id = colInt(row, 0)
    const b = getBridge()
    const memberRes = b.bridgedExecuteCas(
      'SELECT COUNT(*) FROM __cas_bundle_member WHERE bundle_id = ?1',
      [sqlInt(id)],
    )
    const binaryRes = b.bridgedExecuteCas(
      'SELECT COUNT(*) FROM __cas_bundle_binary WHERE bundle_id = ?1',
      [sqlInt(id)],
    )
    const memberCount = Number(colInt(memberRes.rows[0], 0))
    const binaryCount = Number(colInt(binaryRes.rows[0], 0))
    return {
      id,
      name: colOptText(row, 1),
      setHash: colText(row, 2),
      createdAt: colInt(row, 3),
      lastUsedAt: colInt(row, 4),
      memberCount,
      binaryCount,
    }
  }
  ...
  const bundlesHandler = {
    // bundle-save(name?, set-hash, members) -> u64
    bundleSave(name, setHash, members) {
      ensureCasSchema()
      const b = getBridge()

      // Find existing row by set_hash (FIND_FIRST_BY_HASH_SQL).
      const existing = b.bridgedExecuteCas(
        'SELECT b.id, b.name, b.set_hash, b.created_at, b.last_used_at ' +
          'FROM __cas_bundle b WHERE b.set_hash = ?1 LIMIT 1',
        [sqlText(setHash)],
      )
      let bundleId
      const ts = nowSecs()
      if (existing.rows.length > 0) {
        bundleId = colInt(existing.rows[0], 0)
        b.bridgedExecuteCas(
          'UPDATE __cas_bundle SET last_used_at = ?2 WHERE id = ?1',
          [sqlInt(bundleId), sqlInt(ts)],
        )
      } else {
        const insertRes = b.bridgedExecuteCas(
          'INSERT INTO __cas_bundle(name, set_hash, created_at, last_used_at) ' +
            'VALUES (?1, ?2, ?3, ?3)',
          [
            name == null ? sqlNull() : sqlText(name),
            sqlText(setHash),
            sqlInt(ts),
          ],
        )
        bundleId = BigInt(insertRes.lastInsertRowid ?? 0n)
        for (const m of members ?? []) {
          b.bridgedExecuteCas(
            ...
            [
              sqlInt(bundleId),
              sqlText(m.extensionName ?? ''),
              sqlText(m.contentHash ?? ''),
            ],
          )
        }
      }

      // Alias binding. If `name` is provided, attempt to attach it
      // (idempotent on (name -> bundle_id); errors if `name` is
      // bound to a different bundle).
      if (name != null) {
        const conflictRes = b.bridgedExecuteCas(
          'SELECT bundle_id FROM __cas_bundle_alias WHERE name = ?1',
          [sqlText(name)],
        )
        if (conflictRes.rows.length > 0) {
          const existingBid = colInt(conflictRes.rows[0], 0)
          if (existingBid !== bundleId) {
            throw bundlesErr(
              `bundle-save: alias '${name}' already bound to bundle id ${existingBid}, ` +
                `cannot re-bind to ${bundleId}`,
            )
          }
        } else {
          b.bridgedExecuteCas(
            'INSERT INTO __cas_bundle_alias(name, bundle_id) VALUES (?1, ?2)',
            [sqlText(name), sqlInt(bundleId)],
          )
        }
      }
    ...
    bundleFindByName(name) {
      ensureCasSchema()
      const b = getBridge()
      const res = b.bridgedExecuteCas(
        'SELECT b.id, b.name, b.set_hash, b.created_at, b.last_used_at ' +
          'FROM __cas_bundle b JOIN __cas_bundle_alias a ON a.bundle_id = b.id ' +
    ...
    bundleFindByHashPrefix(prefix) {
      ensureCasSchema()
      const b = getBridge()
      const res = b.bridgedExecuteCas(
        'SELECT id, name, set_hash, created_at, last_used_at ' +
          'FROM __cas_bundle WHERE set_hash LIKE ?1 || ?2',
        [sqlText(prefix), sqlText('%')],
      )
    ...
    bundleList() {
      ...
      // Also flatten aliases: bundle-cli's .bundle list prints
      // every alias on its own line. We expose summaries here and
      // bundle-cli's render picks names up via `name` (the legacy
      // display-name hint column) — the alias-list fetch happens
      // separately in bundle-cli only when needed. For the browser
      // test all we need is for `.bundle save myset` to write the
      // alias and `.bundle list` to surface it: __cas_bundle.name
      // is left null in v4 saves but bundle-cli's print path joins
      // __cas_bundle_alias for the display label. To match that,
      // hydrate `name` from the first alias when the column is null.
      const summaries = res.rows.map(summaryFromRow)
      for (const s of summaries) {
        if (s.name == null) {
          const aliasRes = b.bridgedExecuteCas(
            'SELECT name FROM __cas_bundle_alias ' +
              'WHERE bundle_id = ?1 ORDER BY name ASC LIMIT 1',
            [sqlInt(s.id)],
          )
          if (aliasRes.rows.length > 0) {
            s.name = colText(aliasRes.rows[0], 0)
          }
        }
      }
      return summaries
    ...
    bundleShow(id) {
      ensureCasSchema()
      const b = getBridge()
      const summaryRes = b.bridgedExecuteCas(
        'SELECT id, name, set_hash, created_at, last_used_at ' +
          'FROM __cas_bundle WHERE id = ?1',
        [sqlInt(id)],
      )
      if (summaryRes.rows.length === 0) {
        throw bundlesErr(`bundle-show: no bundle with id ${id}`)
      }
      const summary = summaryFromRow(summaryRes.rows[0])
      if (summary.name == null) {
        ...
        if (aliasRes.rows.length > 0) {
          summary.name = colText(aliasRes.rows[0], 0)
        }
      }
      const memberRes = b.bridgedExecuteCas(
        'SELECT extension_name, content_hash FROM __cas_bundle_member ' +
          'WHERE bundle_id = ?1 ORDER BY extension_name ASC',
        [sqlInt(id)],
      )
      const members = memberRes.rows.map((r) => ({
        extensionName: colText(r, 0),
        contentHash: colText(r, 1),
      }))
      const binaryRes = b.bridgedExecuteCas(
        'SELECT target_triple, binary_path, built_at FROM __cas_bundle_binary ' +
          'WHERE bundle_id = ?1 ORDER BY target_triple ASC',
        [sqlInt(id)],
      )
      const binaries = binaryRes.rows.map((r) => ({
        targetTriple: colText(r, 0),
        binaryPath: colText(r, 1),
        builtAt: colInt(r, 2),
      }))
      return { summary, members, binaries }
    },
    ...
    bundleDelete(id) {
      ...
      // changes counter is 0 if no row matched; bundle-cli treats
      // both as success for idempotency.
      void res
      return undefined
    },

    bundleGc(_policy) {
      // Not exercised by the v1.5 round 3 test path. Throw with a
      // structured error so any caller (none today) gets a clear
      // signal.
      throw bundlesErr(
        'bundles.bundle-gc not yet implemented in browser composed runtime',
      )
    },

    bundleRecordBinary(id, targetTriple, binaryPath) {
      ensureCasSchema()
      const b = getBridge()
      b.bridgedExecuteCas(
        'INSERT OR REPLACE INTO __cas_bundle_binary' +
          '(bundle_id, target_triple, binary_path, built_at) ' +
          'VALUES (?1, ?2, ?3, ?4)',
        [
          sqlInt(id),
          sqlText(targetTriple),
          sqlText(binaryPath),
          sqlInt(nowSecs()),
        ],
      )
    ...
    bundleTouch(id) {
      // Best-effort: swallow errors so a missing-schema state on
      // first call doesn't poison a downstream .bundle list. The
      // touch is only an LRU hint, never load-bearing.
      try {
        ensureCasSchema()
        getBridge().bridgedExecuteCas(
          'UPDATE __cas_bundle SET last_used_at = ?2 WHERE id = ?1',
          [sqlInt(id), sqlInt(nowSecs())],
        )
      } catch (_e) {
        // ignore
      }
  ...
  // exist as documented placeholders for the GC / alias-list paths
