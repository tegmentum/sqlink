...
/// Bytes-fetch helper used by the auto-resolve fallthrough
/// (Phase 4 CAS walk). `.sqlink bundle` lives in
/// extensions/sqlink-meta-cli now and uses its own file:// read
/// path; it doesn't go through here.
///
/// Strategy:
///   1. If `source_uri` is `file://PATH` or a bare path, read
///      directly. (Empty source_uri skips this step.)
